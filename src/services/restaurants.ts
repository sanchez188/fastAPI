...
export class RestaurantService {
  ...
  async getRestaurants(filters: RestaurantQuery = {}): Promise<{
    ...
    try {
      ...
      const formattedResults = data.map((restaurant: Restaurant) => {
        const rating = restaurant.rating ? `⭐ ${restaurant.rating}/5` : '⭐ Sin calificación';
        const phone = restaurant.phone ? `📞 ${restaurant.phone}` : '';
        const address = restaurant.address ? `📍 ${restaurant.address}` : '';
        const hours = restaurant.opening_hours ? `🕐 ${restaurant.opening_hours}` : '';
        const payments = restaurant.payment_methods && restaurant.payment_methods.length > 0 
          ? `💳 ${restaurant.payment_methods.join(', ')}` 
          : '';
        const services = restaurant.service_modes && restaurant.service_modes.length > 0 
          ? `🛎️ ${restaurant.service_modes.join(', ')}` 
          : '';
        const email = restaurant.email ? `📧 ${restaurant.email}` : '';
        const mapsUrl = restaurant.google_maps_url ? `🗺️ ${restaurant.google_maps_url}` : '';
...
🆔 ID: ${restaurant.id}
      ...
      const resultText = `
📋 **Restaurantes Encontrados** (${data.length} resultado${data.length !== 1 ? 's' : ''})
...
💡 *Tip: Puedes usar filtros como nombre, calificación mínima/máxima, métodos de pago o modos de servicio para refinar tu búsqueda.*
  ...
  async getRestaurantById(id: string): Promise<{
    ...
    try {
      ...
      const rating = restaurant.rating ? `⭐ ${restaurant.rating}/5` : '⭐ Sin calificación';
      const phone = restaurant.phone ? `📞 **Teléfono:** ${restaurant.phone}` : '';
      const address = restaurant.address ? `📍 **Dirección:** ${restaurant.address}` : '';
      const hours = restaurant.opening_hours ? `🕐 **Horarios:** ${restaurant.opening_hours}` : '';
      const payments = restaurant.payment_methods && restaurant.payment_methods.length > 0 
      ...
      const mapsUrl = restaurant.google_maps_url ? `🗺️ **Google Maps:** ${restaurant.google_maps_url}` : '';
      const created = restaurant.created_at ? `📅 **Creado:** ${new Date(restaurant.created_at).toLocaleDateString()}` : '';
